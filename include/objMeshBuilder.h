...
private:
	bool compare_elements(element_data& rhs, element_data& lhs);
	void processMaterialFile(const std::string&);
	bool find_file(const boost::filesystem3::path&, const std::string&,
...
template<typename T>
void objMeshBuilder::iterate_line(const std::string& line,
		const boost::regex& expression, std::vector<T>& vec) {
